...
class Twist extends Component {
    ...
    componentDidMount() {
        this.getComments()
        this.isOwner(this.state.twist.id_user)
    }

    //verify if user is owner of something
    isOwner()
    {
        return (Cookies.get('id_user')===this.state.twist.id_user)
    }


    getProfile(){
            this.props.showProfile(this.state.twist.login)
            console.log('twist.js getprofile called')
    }
    ...
    render() {
        ...
        <div className="twist">
            <Media className="twist__header">
                ...
                <Media.Body>
                    <div >
                        <Link  to={links.profile} onClick={this.getProfile}
                               className="text--blue text--bold text--small">{twist.firstname} {twist.familyname}</Link>
                    ...
                    <CommentList showProfile={this.props.showProfile}  className="twist__comment-list" twistId={twist.id_message}  refreshList={this.getComments} comments={twist.comments}/>
                </Media.Body>
            </Media>
